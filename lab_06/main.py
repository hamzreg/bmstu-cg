...
def seed_fill(stack, colour, pause):
    ...
    while stack:
        ...
        while image_canvas.get(x, y) != edge_colour and image_canvas.get(x, y) != colour[0]:
            image_canvas.put(colour[1], (x, y))
            x += 1
        rx = x - 1
        image_canvas.put(colour[1], (now_point[0] + 1, y, rx + 1, y + 1))
        ...
        for i in [1, -1]:
            ...
            while x <= rx:
                flag = 0
                while image_canvas.get(x, y) != edge_colour and image_canvas.get(x, y) != colour[0] and x <= rx:
                    flag = 1
                    x += 1
                ...
                if flag:
                    ...
                    flag = 0
                xi = x
                while (image_canvas.get(x, y) == edge_colour or image_canvas.get(x, y) == colour[0]) and x < rx:
                    x += 1
...
def clear():
    ...
    image_canvas.put("#ffffff", to = (0, 0, Graphic.canvas_width, Graphic.canvas_heigth))
...
def draw_edge(edge):
    ...
    for point in edge:
        image_canvas.put("#000000", (point[0], point[1]))
...
def add_seed_dot(x, y):
    ...
    #if len(stack) == 1:
    #     draw_point(stack[0][0], stack[0][1], "white")
    # colour = get_colour()
